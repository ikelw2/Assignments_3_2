...
internal static class SpectrePreloaded
{
    ...
    //===========================================================================
    public static double AskUserForDouble(string prompt = "Enter a Double") 
    {
        double input = AnsiConsole.Prompt(
        new TextPrompt<double>($"{prompt}: ")
            .ValidationErrorMessage("[red]Invalid input.[/] Please enter a valid decimal number."));
            //.Validate(x =>
            //{
            //    return x switch
            //    {
            //        < 0.01 => ValidationResult.Error($"[red]Must be greater than 0.01[/]"),
            //        > 10000000.0 => ValidationResult.Error($"[red]Must be less than {maxVal}[/]"),
